...
void RecognizeCommands::getPreviousResultScores(int* scoresCollect)
{
  int32_t average_scores[kCategoryCount];
    PreviousResultsQueue::Result previous_result = previous_results_.from_front(previous_results_.size() - 1);
    const int8_t* scores = previous_result.scores;
    for (int i = 0; i < kCategoryCount; ++i){
      scoresCollect[i] = scores[i];
    }
